...
do_start() {
    if [ ! -f "$PREPARE_DB_DONE_FILE" ]; then
        ...
        if [[ "$PROFILE" =~ "Sensor" ]]; then
            echo "CALL sensor_update ('admin','$SENSOR_ID','$ADMIN_IP','$HOSTNAME',5,40001,'$DIFF_TZ','','','$VERSION',1,1,1,1,1);" | ossim-db alienvault >> /var/log/alienvault/update/prepare-db.log 2>&1
