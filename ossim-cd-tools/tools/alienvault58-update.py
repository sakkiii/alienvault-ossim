...
class Update():
    ...
    def check_vuln_scan_running(self):
        self.logger.info("Checking for running scan")
        command = "ps -aux | grep openvas | grep -i handling | grep -v grep | wc -l"
        try:
            num_scanners = int(subprocess.check_output(command, shell=True))
        except subprocess.CalledProcessError as e:
            print(e.output)
            self.control.conditional_raw_input("OpenVAS is running an scan.")
            raise SystemExit(EXIT_CODE_ON_ERROR)
        if num_scanners > 0:
