...
sub update_thread {
    ...
    debug_log("alienvault58-update.py -v $args |");
    open( F, "alienvault58-update.py -v $args |" );

    while (<F>) {
        chomp;
        AV::CC::SharedData->set( 'update', "$_" );
        system("echo \"$_\" >> /var/log/alienvault-center_update.log");
        console_log_file("Thread alienvault58-update.py -> $_");
        console_log("Thread alienvault58-update.py -> $_") if $0 !~ /av-centerd/ ;
    }
