...
def get_conf_network_interfaces(system_ip, root=None,store_path = False):
    ...
    try:
        ...
        rt = False
        result = "Can't obtain network configuracion: " + str(e)
    return (rt, result)
...
def set_conf_iface(system_ip,iface,ipaddr=None,netmask=None,gateway=None):
    ...
    try:
        ...
        if (ipaddr or netmask or gateway) is None:
            raise Exception("No parameter to change")
        if netmask:
        ...
        if rc != True:
            raise Exception("Can't configure %s iface with ipaddr => %s netmask => %s msg:%s" % (iface,ipaddr,netmask,msg))
    except Exception,e:
        rc =  False
        msg = "Can't configure interface: " + str(e)
    return (rc, msg)
...
def set_interfaces_roles(system_ip,interface_roles):
    ...
    ip_interfaces = [data['ipv4']['address'] for iface, data in future_net_status.iteritems() if
                     'ipv4' in data and data['ipv4']['address'] is not None and data['role'] != 'disabled' and data['role'] != 'monitoring']
