...
class Vuln_Nessus_Plugins (Base):
    ...
    cve_id = Column('cve_id', VARCHAR(512), primary_key=False)
...
class Hids_Agents(Base):
    __tablename__ = 'hids_agents'
    sensor_id = Column('sensor_id', VARBINARY(16), primary_key=True, nullable=False)
    agent_id = Column('agent_id', VARCHAR(5), primary_key=True, nullable=False)
    agent_name = Column('agent_name', VARCHAR(128))
    agent_ip = Column('agent_ip', VARCHAR(32))
    agent_status = Column('agent_status', TINYINT(1))
    host_id = Column('host_id', BINARY(16))
    ...
    def get_status_integer_from_string(status_str):
        """Returns the status integer value from a string value"""
        status_str = status_str.lower()

        if status_str in Hids_Agents.AGENT_STATUS_MAP:
            return Hids_Agents.AGENT_STATUS_MAP[status_str]
        else:
            return 0

    @staticmethod
    def get_status_string_from_integer(status):
        """Returns the status description given a status integer"""
        status_str = 'Unknown'

        if status in Hids_Agents.AGENT_STATUS_MAP.values():
            for status_key, status_value in Hids_Agents.AGENT_STATUS_MAP.iteritems():
                if status_value == status:
                    status_str = status_key.capitalize()
