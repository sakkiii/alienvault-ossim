				...
				if ($ref == 'Alarm' or $ref == 'Event')
				{
					...
					if ($action == 'newincident')
					{
						if($ref == 'Alarm')
						{
                            ...
                            Incident_ticket::insert($conn, $incident_id, "Open", $priority, $transferred_user, "<a target=\"_blank\" href=\"https://$frameworkd_address/ossim/#analysis/alarms/$alarm_id\">Link to Alarm</a>",'',NULL,array(),array(),false);
						}
						else
						{
...
elseif ($action == 'delete_ticket') /* Delete a ticket */
{
	...
	else
	{
		...
		else
		{
			...
			Incident_ticket::delete($conn, $ticket_id, $incident_id);
