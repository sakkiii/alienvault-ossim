...
?>
    ...
    <head>
        ...
        <script language="javascript">
            ...
            var alarms_total = 0;
            var alarm_session_db = new av_session_db('alarm_db_datatables');
            ...
            function alarm_action(action_number, open_msg) {
                ...
                {
                    $('#delete_data').html("<?php echo _("Applying action to selected alarms ...") ?>");
                    $('#info_delete').show();
                    ...
                    $.ajax(
                        {
                            ...
                            {
                                $('#info_delete').hide();
                                alarm_notification(textStatus, 'nf_error');
                            }
                        });
                });
            }

            function add_value_alarm_session_db(backlog_id) {
                //Reset alarm_session_db
                this.alarm_session_db.clean_checked();

                var final_backlog_id = "check_" + backlog_id;
                this.alarm_session_db.save_check(final_backlog_id);
            }

            function tray_delete(backlog_id) {
                //Reset alarm and add backlog id to alarm session db
                add_value_alarm_session_db(backlog_id);
            ...
            //Show 'Select All' link in the page where it was clicked
            function toggle_select_all_link() {
                ...
                $('#c_alarms_selected').text(selected_checkboxes);
                $('#c_alarms_total').text(alarms_total);

                if (selected_checkboxes > 0 && checkbox_count == selected_checkboxes) {
                ...
                if( selection_type == 'all') {
                    toggle_checkboxes(true, false);
                    $('#allcheck').prop('checked', true);
                }
            }

            function chk_actions() {
                var selected_alarms = get_alarms_checked(true);

                if (selected_alarms.length > 0 || selection_type == 'all')
                {
            ...
            //Select all alarms in the current page
            function check_all(that) {
                if( selection_type == 'all')
                {
                    selection_type = 'manual';
                    toggle_checkboxes(false, false);

                }else{
                    that.checked ? $('#selectall').show() : $('#selectall').hide();
                    toggle_checkboxes(that.checked, true);
                }

                chk_actions();
                toggle_select_all_link();
            }


            //Select all alarms for all pages
            function select_all() {
                $('#selectall').hide();
                $('#allcheck').prop('checked', true);
            ...
            function toggle_checkboxes(status, session_store_save) {
                $("input[type=checkbox].alarm_check").each(function() {
                    if (this.id.match(/^check_[0-9A-Z]+/))
                    {
                        if(!$(this).prop('disabled'))
            ...
            function manage_check(input) {
                ...
                else
                {
                    ...
                    //All alarms were selected but we unchecked one
                    //We need to retrieve the selected alarms in the current page
                    if (selection_type == 'all'){
                        selection_type = 'manual';

                        var checked_alarms = get_alarms_checked(false);

                        checked_alarms.forEach(function(value){
                            __self.alarm_session_db.save_check(value);
                        });
                    }
                }
            }

            function get_alarms_checked(remove_prefix) {
                ...
                $(".alarm_check").each(function()
                {
                    ...
                    if(!$(this).prop('disabled'))
                    {
                        if (alarm_session_db.is_checked(id) || $(this).prop('checked'))
                        {
                            $(this).prop('checked', true);
            ...
            function save_alarm_checked() {
                var checked = get_alarms_checked(true);
            ...
            function check_background_tasks(times) {
                ...
                $.ajax({
                    ...
                    success: function(data)
                    {
                        ...
                        {
                            alarm_notification('<?php echo _("Unable to check background tasks") ?>', 'nf_error');
                        }
                        else
                        {
                            if(data.bg)
                ...
                alarm_table = $('.table_data').dataTable(
                    {
                        ...
                        "fnDrawCallback" : function(oSettings)
                        {
                            ...
                            alarms_total = oSettings._iRecordsTotal;
                            load_handlers();
                            chk_actions();
                            toggle_select_all_link();
                        },
            ...
            // Redraw table when Manage tags lightbox is closed
    ...
    {
        $config_nt = array(
            'content' => $_SESSION["_delete_msg"],
            'options' => array (
                'type'          => 'nf_warning',
                'cancel_button' => true
            ),
            'style'   => 'width: 80%; margin: 15px auto; text-align: left;'
        );
    ...
    <!-- ALARM LIST -->
    <div id='alarm_list'>
        <div class="hidden" id="selectall" style="width: 420px; margin: auto; position: relative;">
            <div style="position: absolute; top: 10px; left: 0px; right: 0px; text-align: center; z-index:100;">
                <?=sprintf(_("You have selected %s alarms on this page."),'<span id="c_alarms_selected"></span>')?>
                <a href="javascript:select_all()">
                    <?=sprintf(_("Select all %s alarms."),'<span id="c_alarms_total"></span>')?>
                </a>
            </div>
        </div>
        ...
        <table class='table_data'>
            <thead>
            <tr>
                <th class="center" id="chkall"><input type="checkbox" id="allcheck" name="allcheck" onclick="check_all(this)"></th>
