...
if(Session::is_pro()) {
    ...
    else $pdf->Image("../pixmaps/logo_ossimpdf.png",10,11,40);
}
else {
...
if ($feed)
{
    if ($ipl=="all") {
        $query ="select distinct res.hostIP, HEX(res.ctx) AS ctx, res.service, res.port, res.protocol, res.app, res.risk, res.scriptid, v.name, res.msg, rep.sid
                    ...
                    from vuln_nessus_latest_results res
                    LEFT JOIN vuln_nessus_plugins_feed as v ON v.id=res.scriptid, vuln_nessus_latest_reports rep
                    where falsepositive='N' 
                    and res.hostIP=rep.hostIP
                    and res.ctx=rep.ctx
                    and res.username=rep.username
...
foreach ($arrResults as $hostIP_ctx=>$scanData) {
    ...
    foreach($scanData as $vuln) {
        ...
        if ($vuln['sid']<0)
        {
            $plugin_info = $dbconn->execute("SELECT t2.name as pfamily, t3.name as pcategory, t1.copyright, t1.summary, t1.version, t1.cve_id 
