...
trait ScheduleSave {
    ...
    public function submit_scan() {
        ...
        if($parameters["schedule_type"] == 'NW') {
            ...
            $requested_run = new DateTime('@' . (mktime($hour,$min,0,$month,1,$year) - $tztimediff));
            $requested_run->modify('-1 day');
            $requested_run = $requested_run->format('YmdHi00');;
        }
        else{
