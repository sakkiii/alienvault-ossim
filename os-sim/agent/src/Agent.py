...
class Agent:
        ...
        for name, path in self.conf.hitems("plugins").iteritems():
            ...
            if os.path.exists(path):
                ...
                if not plugin.has_section('config'):
                    logger.error(
                        "Plugin without [config] section?. If the plugin has the [config] section, please verify the encoding ")
                    continue
                ...
                if os.path.exists(custompath):
                    ...
                    for item in custom_plug.hitems("DEFAULT"):
                        ...
                        if new_value != old_value:
                            ...
                            logger.warning("Loading custon value for %s--->%s. New value: %s - Old value: %s" % (
                            "DEFAULT", item, new_value, old_value ))
                    ...
                    for section in custom_plug.sections():
                        for item in custom_plug.hitems(section):
                            ...
                            if new_value != old_value:
                                ...
                                logger.warning("Loading custon value for %s--->%s. New value: %s - Old value: %s" % (
                                section, item, new_value, old_value))
    ...
    def connect_framework(self):
        ...
        if self.__frameworkConnection is None:
            self.__frameworkConnection = FrameworkConn(self.conf,
                                                       frmk_tmp_id,
                                                       frmk_tmp_ip,
                                                       frmk_tmp_port,
                                                       self.__sensorID,
                                                       self.__systemUUIDFile)
        elif not self.__frameworkConnection.get_alive():
            tryConnect = True

        if tryConnect:
            if self.__frameworkConnection.connect(attempts=3,
                                                  wait=30):
                logger.info("Control Framework (%s:%s) is now enabled!" % ( frmk_tmp_ip, frmk_tmp_port ))
                self.__frameworkConnection.framework_control_messages()
    ...
    def init_plugins(self):
        ...
        for plugin in self.__plugins:
            if plugin.get("config", "type") == "detector":
                ...
                if plugin.get("config", "source") == "json":
                    ...
                    try:
                        ...
                        logger.warning("Unable to load wmi credentials file %s, disabling wmi collection." % (
                        plugin.get("config", "credentials_file") ))
    ...
    def terminate(self, sig, params):
        ...
        if not self.getShutDownRunning():
            ...
            logger.info(
                "WARNING: Shutdown received! - We can't process it because another shutdonw process is running!")
    ...
    def __readOuptutServer(self):
        ...
        if self.conf.has_section("output-server"):
            if self.conf.getboolean("output-server", "enable"):
                ...
                self.__outputServerConnection = ServerConn(server_ip=server_ip,
                                                           server_port=server_port,
                                                           priority=server_priority,
                                                           plugins=self.__plugins,
                                                           sensor_id=self.__sensorID,
                                                           sensor_id_file=self.__systemUUIDFile)
    ...
    def __check_server_status(self):
        ...
        logger.info("Check status configuration: Time between checks: %s - max stop counter: %s" % (
        timeBeetweenChecks, maxStopCounter ))
        #priority 0,1,2,3,4,5
        while self.__keep_working:
            if self.__outputIDMConnection is not None and not self.__outputIDMConnection.get_alive():
                self.__outputIDMConnection.connect()
            if not self.__outputServerConnection or not self.__outputServerConnection.get_alive():
                self.__stop_server_counter += 1

            if self.__stop_server_counter >= maxStopCounter:
                if self.__outputServerConnection:
                    logger.info(
                        "Server %s:%s has reached %s stops, trying to reconnect!" % (self.__outputServerConnection.ip,
                                                                                     self.__outputServerConnection.port,
                                                                                     maxStopCounter))
                ...
                if self.__keep_working:
                    time.sleep(3)
            if self.__outputServerConnection and self.__outputServerConnection.get_alive():
