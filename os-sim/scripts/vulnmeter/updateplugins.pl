...
if (!$is_sensor && !$is_framework) {
    ...
    if (-e "/usr/sbin/greenbone-nvt-sync" ) {
        logwriter( "Updating OpenVAS plugins...", 4 );
        my $ocommand = system ("sudo /usr/sbin/greenbone-nvt-sync --wget > /var/tmp/update_scanner_plugins.log 2>&1");
        ...
        if ( -e "/etc/init.d/openvas-manager" && $ocommand == 0 ) {
           ...
           logwriter( "Executing openvas rebuild", 4 );
           system( "sudo /usr/sbin/openvasmd --rebuild >> /var/tmp/rebuild_nvt_cache.log 2>&1" );
        }
...
if (-e $nessus_vars{'nessus_updater_path'}) {
    ...
    $CONFIG{'NESSUSUPDATEPLUGINSPATH'} = ($nessus_vars{'nessus_path'} =~ /nessus/) ? "/usr/sbin/nessus-update-plugins" : "/usr/sbin/greenbone-nvt-sync";
}
...
sub update_openvas_plugins {
	...
	t1.version, t2.id, t2.version, t2.custom_risk, t2.risk, t1.cve_id, t1.bugtraq_id, t1.xref, t1.cvss_base
	...
	$pluginid, $pluginversion, $plugin_crisk, $p_risk, $pcve_id, $p_bug, $p_xref, $cvss_base )= $sth_sel->fetchrow_array ) {
    ...
    if ($pname ne "" && $pfamily ne "" && $pcategory ne "") {
        ...
        if($cvss_base =~ /\d+\.\d+/) {
            ...
            elsif( int($cvss_base) >= 7 && int($cvss_base) < 9 ) {
                $risk = 2;
            }
            elsif( int($cvss_base) >= 4 && int($cvss_base) < 7 ) {
                $risk = 3;
            }
            elsif( int($cvss_base) > 0 && int($cvss_base) < 4 ) {
                $risk = 6;
            }
            elsif( int($cvss_base) == 0 && $pcve_id ne "" ) {
                $risk = 6;
            }
            else {
                $risk = 7;
    	...
    	if ( !defined( $pluginid ) || $pluginid eq "" ) {
    	    ...
    	    $plugins{$pid}{'do'} = "update";
                if ($pluginversion ne $pversion || $p_risk ne $risk) {
