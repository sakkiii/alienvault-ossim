...
if ($nessus !~ /omp\s*$/) {
    ...
    while(<PLUGINS>){
        ...
        if ($#plugin_data > 3) {
            ...
            my $temp_plugin_id = $id;
                ...
                if ($risk eq "Veryhigh") { $risk_level = 4 }
                }
    ...
    print "\nGetting nessus NVTS details, please wait \n";
    ...
    foreach my $nvt (@items) {
        ...
        if ($nvt->{'cvss_base'} eq "" || ref($nvt->{'cvss_base'}) eq 'HASH')
        {
            $risk_level = 0;
        }
        elsif($nvt->{'cvss_base'} =~ /\d+\.\d+/)
        {
            if( int($nvt->{'cvss_base'}) >= 9 ) {
                $risk_level = 4;
            }
            elsif( int($nvt->{'cvss_base'}) >= 7 && int($nvt->{'cvss_base'}) < 9) {
                $risk_level = 4;
            }
            elsif( int($nvt->{'cvss_base'}) >= 4 && int($nvt->{'cvss_base'}) < 7 ) {
                $risk_level = 3;
            }
            elsif( int($nvt->{'cvss_base'}) > 0 && int($nvt->{'cvss_base'}) < 4 ) {
                $risk_level = 1;
            }
            elsif( int($nvt->{'cvss_base'}) == 0 && $nvt->{'cve_id'} ne 'NOCVE' ) {
                $risk_level = 1;
            }
            else {
                $risk_level = 0;
        ...
        else
        {
            ...
            if ($risk eq "Medium")   { $risk_level = 3 }
            if ($risk eq "High")     { $risk_level = 4 }
            if ($risk eq "Critical") { $risk_level = 4 }
        }
...
chop($query);
$query.= " ON DUPLICATE KEY UPDATE name=plugin_sid.name;";
