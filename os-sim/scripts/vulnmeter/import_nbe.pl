...
sub update_ossim_incidents {
        ...
        if (defined($id_inc) && $id_inc ne "") {
            ...
            my $priority = calc_priority($risk);
...
sub get_cve {
    my $scan_id = $_[0];
    my $sid = $_[1];
    my $vuln_table = ($sid eq '-1') ? "vuln_nessus_plugins_feed" : "vuln_nessus_plugins";
    my $sql = qq{ SELECT cve_id FROM $vuln_table t1 WHERE oid = '$scan_id' };
