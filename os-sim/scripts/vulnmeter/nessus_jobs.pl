...
sub check_update_running {
  my $program_name = 'alienvault58-update.py';
  my $running_instance = `ps -feaww | grep "python /usr/bin/$program_name" | grep -v grep | grep -v ps | wc -l`;
  ...
  if ( $running_instance > 0 ) {
      ...
      logwriter( "Scanning queue checking skipped.  A system update in progress.", 4 );
      exit;
  }
}
...
sub update_host_record {
    ...
    $sql = "INSERT INTO vuln_host_stats ( report_id, host_id, hostip, hostname, scantime, localChecks, access_rating,
        vCritical, vHigh, vMed, vMedLow, vLowMed, vLow, vInfo ) VALUES (
...
sub pop_hosthash {
	...
	#IDENTIFY IF LOCAL CHECKS FAILED
        ...
        if ($CONFIG{'NESSUSPATH'} !~ /omp\s*$/) {
            ...
            if ($risk < 0) {
                $risk=2  if($risk_factor eq "Serious");
...
sub gen_sched_next_scan {
    ...
    if ($schedule_type eq "O") {
        ...
        system(@reschedule_args) == 0 or die "ERROR: failed to re-schedule OpenVAS schedule with ID $schedid!";
    } else {
...
sub update_ossim_incidents {
        ...
        $risk = 9 - $risk; # convert into ossim risk
        ...
        if ( noEmpty($id_inc) ) {
            ...
            my $priority = calc_priority($desc);
...
#I'm sorry
    ...
    if(noEmpty($credentials{'smb_credential'}) && $credentials{'smb_credential'} =~ /^\w+\-\w+\-\w+\-\w+\-\w+$/) {
        ...
        logwriter("smb_credential: <smb_credential id='".$credentials{'smb_credential'}."'/>", 4);
    }
    
    if(noEmpty($credentials{'ssh_credential'}) && $credentials{'ssh_credential'} =~ /^\w+\-\w+\-\w+\-\w+\-\w+$/) {
        $ls_credentials .= "<ssh_credential id='".$credentials{'ssh_credential'}."'/>";
        logwriter("<ssh_credential id='".$credentials{'ssh_credential'}."'/>", 4);
    }
...
sub execute_omp_command {
    ...
    do {
        ...
        if ($debug_file ne "") {
            ...
            system ("cat $file_omp_command >> '$debug_file'");
    ...
    # Reset error failure
    if ( $retry < $maxretries && $job_id > 0 ) {

        if($nessushost eq "") {

            my $id_sensor = '';
            $sql = qq{ SELECT hostname
                       FROM vuln_nessus_servers t1 LEFT JOIN vuln_jobs t2 on t1.id = t2.scan_SERVER
                       WHERE t2.id = '$job_id' };

            $sth_sel = $dbh->prepare( $sql );
            $sth_sel->execute;
            $id_sensor = $sth_sel->fetchrow_array;
            logwriter("Sensor ID: $id_sensor", 4);
            $nessushost = get_sensor_ip_by_id($id_sensor);
            logwriter("Sensor IP: $nessushost", 4);
        }

        $sql = qq{ UPDATE vuln_jobs SET meth_Wcheck='Scan Server Selected: $nessushost<br />' WHERE id='$job_id' };
        safe_db_write ( $sql, 1 );
    }
...
sub get_results_from_xml {
    ...
    my $total_records = 0;
    my (@items, @nvt_data, $host, $service, $scan_id, $description, $app, $proto, $port, $risk_factor, $cve, @issues, %hostHash, %resultHash, $report_id, $xml, %apps_by_port_protocol, %plugins_desc);
    ...
    foreach my $result (@items) {
            ...
            if($result->{"nvt"}->{"cvss_base"} =~ /\d+\.\d+/) {
                if( int($result->{"nvt"}->{"cvss_base"}) >= 9 ) {
                    $risk_factor = "High";
                }
                elsif( int($result->{"nvt"}->{"cvss_base"}) >= 7 && int($result->{"nvt"}->{"cvss_base"}) < 9) {
                    $risk_factor = "High";
                }
                elsif( int($result->{"nvt"}->{"cvss_base"}) >= 4 && int($result->{"nvt"}->{"cvss_base"}) < 7 ) {
                    $risk_factor = "Medium";
                }
                elsif( int($result->{"nvt"}->{"cvss_base"}) > 0 && int($result->{"nvt"}->{"cvss_base"}) < 4 ) {
                    $risk_factor = "Low";
                }
                elsif( int($result->{"nvt"}->{"cvss_base"}) == 0 && $cve ne 'NOCVE' ) {
                    $risk_factor = "Low";
                }
            }
    ...
    if ($delete_credentials == TRUE) {
        foreach my $tcred ( keys %credentials ) {
            execute_omp_command("<delete_credential credential_id='".$credentials{$tcred}."'/>", $sensor_id, 0);
...
sub get_task_data {
    ...
    if ( noEmpty( $xml->{"target"}->{'ssh_credential'}->{'id'}) ) {
	    $result{'credentials'}{'ssh_credential'} = $xml->{"target"}->{'ssh_credential'}->{'id'};
    }
    
    # get smb credential
    
    if ( noEmpty( $xml->{"target"}->{'smb_credential'}->{'id'} ) ) {
	    $result{'credentials'}->{'smb_credential'} = $xml->{"target"}->{'smb_credential'}->{'id'};
    }
